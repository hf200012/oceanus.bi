        ...
        <el-form-item :label="$t('chart.chartType')+':'">
          <div class="chart-type-list">
            <span
              ...
              :key="item.type"
              :class="{activedIcon :item.type===chartType, disabledIcon: !isUsable(item)}"
              @click="switchChartType(item)"
            >
              <el-tooltip :content="item.name +': '+item.matchRule.desc" placement="top">
                {{isUsable(item)? item.icon : (item.icon + '_disabled')}}
                <svg-icon
                  class="icon"
                  :icon-class="isUsable(item)? item.icon : (item.icon + '_disabled')"
                />
              </el-tooltip>
            </span>
          </div>
...
import MapChart from "@/widgets/chinamap"
...
export default {
  ...
  computed: {
    ...
    chartData() {
      return this.currentType.dataTransfer
        ? this.currentType.dataTransfer(this.data, this.schema)
        : this.data;
    },
    currentType() {
  ...
  watch: {
    allSelected: {
      ...
      handler() {
        if (
          !this.currentType.matchRule.isUsable(
            store.state.dimensions,
            store.state.caculCols
          )
        ) {
  ...
  methods: {
    isUsable(chart) {
      return chart.matchRule.isUsable(
        store.state.dimensions,
        store.state.caculCols
      );
    },
    switchChartType(chart) {
      if (
        !chart.matchRule.isUsable(store.state.dimensions, store.state.caculCols)
      ) {
        return;
      }
