<template>
  <el-form-item :label="$t('chart.filters')">
    <el-tag
      ...
      :key="index"
      closable
      @close="handleClosefilter(index)"
      @click="handleEditFilter(item)"
    >{{ generateFilterSentence(item) }}</el-tag>
    <el-button
      :disabled="disabled"
      type="primary"
      size="mini"
      ...
      <el-form label-width="150px">
        <el-form-item :label="$t('chart.filterField')" class="el-form-item">
          <el-select
            v-model="filteCol"
            size="mini"
            :placeholder="$t('chart.selectFilterField')"
            style="width: 220px;"
          >
            <el-option
              v-for="item in sharedState.allCols"
              :key="item.Column"
              :label="item.cname"
              :value="item.Column"
            />
        ...
        <el-form-item :label="$t('chart.filterOperator')" class="el-form-item">
          <el-select
            ...
            size="mini"
            :placeholder="$t('chart.selectFilterOperator')"
            style="width: 220px;"
          >
            <el-option
              v-for="item in filterOperatorOpts"
              :key="item.name"
              :label="`${item.name}`"
              :value="item.operator"
            />
        ...
        <el-form-item
          v-if="currentOperatorParamNum===-1"
          :label="$t('chart.comparedValue')"
          class="el-form-item"
        >
          <el-tag
          ...
          <el-button size="mini" type="primary" @click="handleAdd">Add</el-button>
        </el-form-item>
        ...
        <el-form-item v-else :label="$t('chart.comparedValue')" class="el-form-item">
          ...
          <el-input
            v-show="currentOperatorParamNum===2"
            v-model="value2"
            style="width: 220px;"
            type="text"
          />
...
export default {
  ...
  computed: {
    currentOperatorParamNum() {
      const a = getFilterOperator().find(
        (item) => item.operator === this.filterOperator
      );
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      const filterObj = {
        ...
        colType: this.sharedState.allCols.find(
          (item) => item.Column === this.filteCol
        ).Type,
      ...
      const filterIndex = this.currentFilters.findIndex((item) => {
        return (
          item.filteCol === this.filteCol &&
          item.filterOperator === this.filterOperator
        );
      });
      if (filterIndex >= 0) {
    ...
    needQuotation(col) {
      const colType = this.sharedState.allCols.find(
        (item) => item.Column === col
      ).Type;
      if (colType.indexOf("(") >= 0) {
        return dataType.find(
          (type) => type.name === colType.split("(")[0].toLowerCase()
        ).needQuotation;
      } else {
        return dataType.find((type) => type.name === colType.toLowerCase())
          .needQuotation;
      }
    },
