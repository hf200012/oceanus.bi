...
export function buildSqlSentence({ dataSrc, selectedCalcul, selectedDimension, orderByStrs, filterStr, limit }) {
  ...
  fields = allSelected.map(field => {
    ...
    if (calculField && calculField.calculFunc !== 'none' ) {
      if(calculField.calculFunc == 'distinct') {
        return `count(distinct ${calculField.Column}) as ${calculField.Column}`
      }else {
...
export function buildFilterSentence(filter) {
  ...
  if (trimColType(filter.colType).needQuotation) {
    valueObj = addQuotation(filter.value,filter)
  }
  ...
  function addQuotation(valueObj,filter) {
    const colType = filter.colType;
    if(colType != 'varchar' && colType != 'varchar2' && colType !='tinytext' && colType !='text' && colType !='mediumtext' && colType !='longtext'){
      return {
